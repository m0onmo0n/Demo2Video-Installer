...
function Enable-LongPaths {
  try {
    ...
    $val     = 0
    try {
      $val = (Get-ItemProperty -Path $regPath -Name $name -ErrorAction Stop).LongPathsEnabled
    } catch {
      # ignore
    }
    if ($val -ne 1) {
...
function Try-Install7Zip {
  ...
  if ($wingetOk) {
    try {
      ...
      if ($p.ExitCode -eq 0 -and (Test-Path $sevenZip)) {
        return $sevenZip
      }
    } catch {
...
function Download-Zip {
  ...
  if ($p.ExitCode -ne 0) {
    throw ('curl.exe failed (exit {0})' -f $p.ExitCode)
  }
...
function Extract-Zip {
  param(
    ...
    [Parameter(Mandatory=$true)][hashtable]$Extractor
  )
...
function Finalize-Folder {
  ...
  if ($rob.ExitCode -ge 8) {
    throw ('robocopy failed (exit {0})' -f $rob.ExitCode)
  }
...
function Run-Installer {
  ...
  if (-not (Test-Path $installer)) {
    throw ('install.bat not found at {0}' -f $installer)
  }
...
$proj = Finalize-Folder -Dest $dest -Keep:$KeepExtracted -ForceExisting:$Force

if (Test-Path $tempZip) {
  try { Remove-Item -Force $tempZip } catch { }
}
